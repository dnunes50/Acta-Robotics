...
// Use service role for writes (bypasses RLS, faster)
...
export async function POST(req: Request) {
  try {
    ...
    if (type === 'budget') {
      await supabase.from('cf_budget_rows').delete().gt('id', 0)
      if (rows?.length) {
      ...
      if (classMap && Object.keys(classMap).length) {
        await supabase.from('cf_class_map').delete().gt('id', 0)
    ...
    if (type === 'extrato') {
      await supabase.from('cf_extrato_rows').delete().gt('id', 0)
      if (rows?.length) {
        const toInsert = rows
